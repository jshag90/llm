...
public class PromptTemplateUtil {
    ...
    public static String getSchemaPrompt(JdbcTemplate jdbcTemplate, String condition){
     ...
     /*   tableSchemas += getTableSchema(jdbcTemplate, "ds_policy_data_protect_log", "데이터 정책 로그 테이블");
        ...
        return "You are a SQL expert. Given the schema below, "
                + tableSchemas +
                "\nGenerate a SQL query for this request: \"" + condition + "\""
                + ".Output only the SQL query without any explanation.";
    }

    private static String getTableSchema(JdbcTemplate jdbcTemplate, String tableName, String tableInfo) {
        String query = "SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE, COLUMN_COMMENT " +
                "FROM INFORMATION_SCHEMA.COLUMNS " +
                "WHERE TABLE_NAME = ?";

        StringBuilder schemaBuilder = new StringBuilder("Here is the schema of the \"" + tableName+"("+tableInfo +")"+ "\" table:\n");
